      ...
      return `
        ...
        product.name
                ...
                <div class="flex-1 min-w-0">
                    <h4 class="text-sm font-bold text-primary truncate">${
                      product.name
                    }</h4>
                    <p class="text-xs text-gray-500">Qty: ${item.quantity}</p>
                </div>
                <div class="text-sm font-bold text-primary">${UI.formatPrice(
...
function initProfilePage(products) {
  ...
  const isLuxury =
    window.location.href.includes("luxury-template") ||
  ...
  if (navAccount && navOrders && sectionAccount && sectionOrders) {
    navAccount.addEventListener("click", (e) => {
      ...
      // Update Active State
      if (isLuxury) {
        navAccount.className =
          "flex items-center gap-4 px-4 py-4 bg-white/5 text-gold border-l-2 border-gold transition-all";
        navOrders.className =
          "flex items-center gap-4 px-4 py-4 text-gray-400 hover:text-white hover:bg-white/5 transition-all border-l-2 border-transparent hover:border-white/20";
      } else {
        navAccount.className =
          "flex items-center gap-3 px-4 py-3 bg-primary text-white rounded-xl font-medium transition-colors";
        navOrders.className =
          "flex items-center gap-3 px-4 py-3 text-gray-600 hover:bg-gray-50 rounded-xl font-medium transition-colors";
      }
      // Show Section
  ...
  orderContainer.innerHTML = user.orders
    .map((order) => {
      if (isLuxury) {
        // Luxury Template Row
        ...
        return `
                <tr class="border-b border-white/5 group hover:bg-white/[0.02] transition-colors">
                    ...
                    <td class="py-6"><span class="text-xs uppercase tracking-wider ${statusColor}">${
          order.status
        }</span></td>
                    <td class="py-6 font-serif text-white">${UI.formatPrice(
                      order.total
                    )}</td>
        ...
        return `
                <tr class="border-b border-gray-50 group hover:bg-gray-50 transition-colors">
                    ...
                    <td class="py-4"><span class="px-3 py-1 ${statusClass} rounded-full text-xs font-bold">${
          order.status
        }</span></td>
                    <td class="py-4 font-bold text-primary">${UI.formatPrice(
                      order.total
                    )}</td>
      ...
      return `
        ...
        product.name
                ...
                <div class="flex-1 text-center sm:text-left">
                    ...
                    <div class="font-bold text-accent">${UI.formatPrice(
                      product.price
                    )}</div>
                </div>
                ...
                <div class="flex items-center gap-4">
                    <div class="flex items-center border border-gray-200 rounded-lg">
                        <button onclick="Cart.updateQuantity(${product.id}, ${
        item.quantity - 1
      })" class="w-8 h-8 flex items-center justify-center hover:bg-gray-50 text-gray-500 transition-colors"><i class="ph ph-minus"></i></button>
                        <span class="w-10 text-center text-sm font-bold text-primary">${
                          item.quantity
                        }</span>
                        <button onclick="Cart.updateQuantity(${product.id}, ${
        item.quantity + 1
      })" class="w-8 h-8 flex items-center justify-center hover:bg-gray-50 text-gray-500 transition-colors"><i class="ph ph-plus"></i></button>
                    </div>
                    <button onclick="Cart.removeItem(${
                      product.id
                    })" class="w-10 h-10 flex items-center justify-center text-gray-400 hover:text-red-500 transition-colors rounded-full hover:bg-red-50">
...
function initProductPage(products) {
  ...
  if (product.discount > 0) {
    ...
    priceEl.innerHTML = `
            <span class="text-red-500 mr-2">${UI.formatPrice(
              discountedPrice
            )}</span>
            <span class="text-gray-400 line-through text-xl">${UI.formatPrice(
              product.price
            )}</span>
        `;
  } else {
  ...
  if (badgesContainer) {
    ...
    if (product.isHot) {
      ...
      if (isLuxury) {
        badgesHtml +=
          '<span class="text-[10px] uppercase tracking-widest text-white bg-red-600 px-2 py-1 font-bold">Hot</span>';
      } else {
        badgesHtml +=
          '<span class="bg-red-500 text-white text-xs font-bold px-2 py-1 rounded-full flex items-center gap-1"><i class="ph-fill ph-fire"></i> HOT</span>';
      }
    }
    if (product.discount > 0) {
      const isLuxury =
...
function initShopPage(products) {
  ...
  const renderPagination = (totalPages) => {
    ...
    } class="w-10 h-10 flex items-center justify-center rounded-lg border border-gray-200 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
